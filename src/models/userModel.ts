...
const _hashing = (password: string) => {
    return bcrypt.hashSync(`${password} + ${config.pepper as string}`, +(config.salt as string, 10))
};
    ...
    async authenticateUser(email: string, password: string): Promise<User | null> {
        try {
            ...
            if (_result.rows.length) {
                ...
                const isPassword = bcrypt.compareSync(`${password}${config.pepper}`, _hashing);
                if (isPassword) {
                    const uInfo = await _connect.query('SELECT id, email, username from users WHERE email=($1)', [email]);
                    return uInfo.rows[0]
                }
            }
            _connect.release();
